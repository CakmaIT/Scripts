...
$initlb = Initialize-LoadBalancer -Address $KempIP -LBPort 443 -Credential $KempCreds
...
$vservers = Get-VirtualService | sort vsaddress
...
$vservers = Get-VirtualService | where {$_.rs}
foreach ($vserver in $vservers)
	{
		$vsnickname = $vserver.nickname
		
		$cells=@("$l_kemp_rsip","$l_kemp_rsport","$l_kemp_RSStatus","$l_kemp_Limit","$l_kemp_weight","$l_kemp_active")
		$kempreport += Generate-HTMLTable "$vsnickname $l_kemp_overview" $cells
				
		$blockstart = 0
		$blockend = 8
		$realserver = $vserver.rs.Split(",")
		do
		{
			$block = $realserver[$blockstart..$blockend]
