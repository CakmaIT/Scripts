		...
		if ($mbxusedbdefault -eq "True" -and $dblimit -eq "Active")
			{
				...
				$limittype = "Database"
				[array]$reportlimits += new-object PSObject -property @{Mailbox="$mbxname";MBXAlias="$mbxalias";LimitType="$Limittype";LimitSize="$limitsize";MailboxSize="$mbxsize";WaringActive="$warningactive";Database=$mbxdatabase}
			}
...
$reportlimits = $reportlimits | where {$_.WaringActive -match "True"}
...
$dismbxs = get-mailboxdatabase | get-mailboxstatistics | Where{ $_.DisconnectDate -ne $null } | select displayName,Identity,disconnectdate,database,totalitemsize
