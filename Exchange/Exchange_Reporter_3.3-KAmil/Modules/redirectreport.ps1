...
$cells=@("$l_redir_mbx","$l_redir_rulename","$l_redir_type","$l_redir_targetaddr")
$redirectreport += Generate-HTMLTable "$l_redir_header2" $cells

$rules = get-mailbox -resultsize unlimited | Sort-Object -Property Name | Get-InboxRule -ea 0 -wa 0 | where {$_.forwardto -or $_.redirectto} 
foreach ($rule in $rules)
	{
		...
		$cells=@("$mbxname","$rulename","$type","$target")
