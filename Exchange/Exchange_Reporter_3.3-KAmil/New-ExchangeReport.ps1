#$cred=Get-Credential
$session=New-PSSession -ComputerName ASG100EXC12
Enter-PSSession $session

Invoke-Command -Session $session {
#--------------------------------------------------------------------------------------
# Exchange Reporter 3.3
# for Exchange Server 2010/2013 und Office 365
# www.frankysweb.de
#
# Gernerating Exchange Reports
# by Frank Zoechling
#
#--------------------------------------------------------------------------------------
...
$host.ui.RawUI.WindowTitle = "Exchange Reporter $reporterversion "
...
write-host "
                    for Exchange Server 2010 / 2013 / 2016 / Office365
...
write-host " Checking for Powershell Version 4.0:" -nonewline
...
$OnlyOffice365 = ($reportsettings | Where-Object {$_.Setting -eq "OnlyUseOffice365"}).Value
...
$IntegrateInExchangeMonitor = ($reportsettings | Where-Object {$_.Setting -eq "IntegrateInExchangeMonitor"}).Value

if ($IntegrateInExchangeMonitor -match "yes")
	{
		$ExchangeMonitorInstallPath = ($reportsettings | Where-Object {$_.Setting -eq "ExchangeMonitorInstallPath"}).Value
	}
...
$origpos.X = 70
	...
	if ($emsversion -notmatch "2010" -and $emsversion -notmatch "2013" -and $emsversion -notmatch "2016")
		{
			$host.UI.RawUI.CursorPosition = $origpos
...
if ($AddPDFFileToMail -match "yes")
	...
	try
		...
		$pdf = &"C:\Program Files\wkhtmltopdf\bin\wkhtmltopdf.exe" --quiet "$installpath\TEMP\PDFReport.htm" "$installpath\TEMP\Report.pdf" | out-null
...
try 
	{
		if ($SMTPAuth -match "yes")
			{
				send-mailmessage -encoding UTF8 -from "Exchange Reporter <$sender>" -to "$Recipient"  -subject "$subject" -smtpserver $mailserver -BodyAsHtml -Body $mailbody -Attachments $files -Credential $smtpcreds
				$host.UI.RawUI.CursorPosition = $origpos
				write-host "Done" -foregroundcolor green
			}
		else
			{
				send-mailmessage -encoding UTF8 -from "Exchange Reporter $sender" -to $Recipient  -subject "$subject" -smtpserver $mailserver -BodyAsHtml -Body $mailbody -Attachments $files
				$host.UI.RawUI.CursorPosition = $origpos
				write-host "Done" -foregroundcolor green
				write-host ""
			}
...
# Report in Exchange Monitor integrieren
#--------------------------------------------------------------------------------------

if ($IntegrateInExchangeMonitor -match "yes")
	{
	write-host " Saving Report for Exchange Monitor:" -nonewline
	$origpos = $host.UI.RawUI.CursorPosition
	$origpos.X = 70
	try
		{
		$lastreport = $mailbody.replace("cid:","")
		$exmonfile = "$ExchangeMonitorInstallPath" + "\HTML\Reporter\lastreport.html"
		$lastreport | set-content $exmonfile
		$picpath = "$ExchangeMonitorInstallPath" + "\HTML\Reporter"
		foreach ($file in $files)
			{
				copy-item $file -Destination $picpath -force
			}
		$host.UI.RawUI.CursorPosition = $origpos
		write-host "Done" -foregroundcolor green
		}
