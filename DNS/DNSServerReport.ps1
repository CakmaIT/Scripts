...
$ac= @()
    ...
    #Write-Host $Comp
         ...
         Try
                {
				$ab=Get-DnsServerForwarder -ComputerName $Comp | Select UseRootHint,EnableReordering,IPAddress,ReorderedIPAddress,@{Label = 'ServerName'; Expression = {$Comp}}
                $ac += $ab
                }
         Catch
                {
		        ...
		        Write-Log -Message $ErrorMessage -LogFile C:\Users\admcicek\Desktop\DNSExport\$Tarlog-DNSServerReport.txt -LogLevel ERROR -ConsoleOutput
                Write-Log -Message $FailedItem -LogFile C:\Users\admcicek\Desktop\DNSExport\$Tarlog-DNSServerReport.txt -LogLevel ERROR -ConsoleOutput 
                                }


}

$ac | Export-Csv C:\Users\admcicek\Desktop\DNSExport\$tarlog-DnsReport.csv
...
$Subject = 'DNS Sunucuları Forwarders Raporu'
...
<br />
<font face=Tahoma>DNS Sunucuları Forwarders Raporu aşağıdaki gibidir.</font><br />
<br />
