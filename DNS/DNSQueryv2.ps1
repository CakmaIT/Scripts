...
$ac2= @()
    foreach ($Comp2 in $Computers){
    #Write-Host $Comp
        foreach ($Sorgu2 in $domainsorgu2){
         Try
                {
				$ab2=Resolve-DnsName $Sorgu2 -Server $Comp2 | Where {$_.Type -eq "A"} | Select Name,IPAddress,@{Label = 'ServerName'; Expression = {$Comp2}}
                $ac2 += $ab2
                }
         Catch
                {
                $ErrorMessage = $_.Exception.Message
                $FailedItem = $_.Exception.ItemName
		        Write-Log -Message $ErrorMessage -LogFile C:\Users\admcicek\Desktop\DNSExport\$Tarlog-2DNShatalari.txt -LogLevel ERROR -ConsoleOutput
                Write-Log -Message $FailedItem -LogFile C:\Users\admcicek\Desktop\DNSExport\$Tarlog-2DNShatalari.txt -LogLevel ERROR -ConsoleOutput 
                                }


}
}
$ac2 | Export-Csv C:\Users\admcicek\Desktop\DNSExport\$tarlog-2.csv

$acarrayoutput2 = $ac2 | ConvertTo-Html -Head $style 
...
<br />
<strong>$acarrayoutput2</strong><br /><br />
<br /></font></h5>
"@
